...
const influencerMetricsService = require('../services/influencer/influencerMetricsService');
...
const getInfluencerDashboard = async (req, res) => {
  try {
    ...
    const influencer = await influencerProfileService.getInfluencerById(influencerId);
    if (!influencer) {
    ...
    const activeCollaborations = await collaborationManageService.getActiveCollaborations(influencerId);
    console.log('Active collaborations:', activeCollaborations);

    // Get pending requests
    const pendingRequests = await collaborationManageService.getPendingRequests(influencerId);
    console.log('Pending requests:', pendingRequests);

    // Get brand invites
    const brandInvites = await collaborationManageService.getBrandInvites(influencerId);
    console.log('Brand invites:', brandInvites);

    // Get sent requests
    const sentRequests = await collaborationManageService.getSentRequests(influencerId);
    ...
    const fullHistory = await influencerProfileService.getCampaignHistory(influencerId);
    const recentCampaignHistory = (fullHistory || [])
      .sort((a, b) => new Date(b.end_date) - new Date(a.end_date))
      .slice(0, 3)
      .map(c => ({
        // Map from influencerProfileService.getCampaignHistory structure
...
const getInfluencerExplorePage = async (req, res) => {
  try {
    const influencers = await influencerProfileService.getAllInfluencers();
    return res.json({ success: true, influencers: influencers || [] });
  } catch (err) {
    console.error('Controller error:', err);
    return res.status(500).json({ success: false, message: 'Error fetching influencers' });
  }
};
...
const getInfluencerProfile = async (req, res) => {
  try {
    ...
    // Get influencer profile data
    const influencer = await influencerProfileService.getInfluencerProfileDetails(influencerId);
    ...
    const metrics = await influencerProfileService.getInfluencerMetrics(influencerId);
    ...
    const bestPosts = await influencerProfileService.getBestPerformingPosts(influencerId);
    ...
    return res.json({ success: true, influencer: formattedInfluencer });
  } catch (error) {
...
const updateInfluencerData = async (req, res) => {
  try {
    ...
    const currentProfile = await influencerProfileService.getInfluencerById(influencerId);
    if (!currentProfile) {
      return res.status(404).json({
        success: false,
    ...
    if (hasChanges) {
      await influencerProfileService.updateInfluencerProfile(influencerId, updateData);
    }
...
const updateProgress = async (req, res) => {
  try {
    ...
    if (reach !== undefined || clicks !== undefined || performance_score !== undefined || conversions !== undefined || engagement_rate !== undefined || conversion_rate !== undefined || impressions !== undefined || revenue !== undefined || roi !== undefined) {
      const CampaignMetrics = require('../models/CampaignMongo').CampaignMetrics;
