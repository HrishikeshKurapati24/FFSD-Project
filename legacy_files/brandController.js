...
const brandController = {
  ...
  async getExplorePage(req, res) {
    try {
      ...
      const allInfluencers = await brandProfileService.getAllInfluencers();
    ...
    try {
      ...
      if (data.socialLinks && Array.isArray(data.socialLinks)) {
        try {
          ...
          await brandProfileService.updateSocialLinks(brandId, socialLinksPayload);
        } catch (error) {
      ...
      const updatedBrand = await brandProfileService.updateBrandProfile(brandId, updateData);
  ...
  async updateSocialLinks(req, res) {
    try {
      ...
      const updatedBrand = await brandProfileService.updateSocialLinks(brandId, socials);
  ...
  async getTopCampaigns(req, res) {
    try {
      const brandId = req.session.user.id;
      const campaigns = await brandProfileService.getTopCampaigns(brandId);
  ...
  async getBrandDashboard(req, res) {
    try {
      ...
      const [brand, stats, activeCampaigns, analytics, campaignRequests, recentCompletedCampaigns, completedProgressCampaigns, influencerRankings, brandProducts] = await Promise.all([
        brandProfileService.getBrandById(brandId),
        brandProfileService.getBrandStats(brandId),
        brandCampaignService.getActiveCampaigns(brandId),
        brandProfileService.getBrandAnalytics(brandId),
  ...
  async getCampaignHistory(req, res) {
    try {
      ...
      const campaigns = await brandCampaignService.getCampaignHistory(brandId);
